...
class TimerScreen extends StatefulWidget {
  ...
  TimerScreen({super.key, required this.timer_value});
...
class _TimerScreenState extends State<TimerScreen>
  ...
  late BreathingPhase currentPhase;
  late Timer cycleTimer;
  late Timer phaseTimer;
  ...
  init() async {
    super.initState();
    remainingTime = widget.timer_value.inSeconds;
  ...
  void startPhaseTimer() {
    phaseTimer = Timer.periodic(Duration(seconds: 3), (timer) {
      setState(() {
        switch (currentPhase) {
    ...
    cycleTimer = Timer.periodic(Duration(seconds: 1), (timer) {
      if (remainingTime > 0) {
        setState(() {
          remainingTime--;
        });
            ...
            children: [
              ...
              Center(
                  child: commonWidget.countDownTimer(
                      context, _controller, widget.timer_value.inSeconds,"breathein",widget.timer_value)),
              SizedBox(
                height: 35.h,
              ),
              Center(child: commonWidget.title_breath(getPhaseText())),
